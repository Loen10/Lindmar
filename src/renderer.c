...
static void create_debug_messenger(struct Renderer *renderer)
{
        ...
        assert_vulkan(func(renderer->instance, &info, NULL,
...
static void select_extent(const VkSurfaceCapabilitiesKHR *capabilities,
        uint32_t width, uint32_t height, VkExtent2D *extent)
{
...
static uint32_t select_image_count(
...
static VkPresentModeKHR select_present_mode(uint32_t count,
        ...
        const struct SwapchainDetails *details, struct Renderer *renderer)
{
        ...
        uint32_t *qindices = get_queue_indices(indices, &qindex_count,
                &shrmode);
        ...
        info.presentMode = select_present_mode(details->present_mode_count,
                details->present_modes);
        info.preTransform = details->capabilities.currentTransform;

        assert_vulkan(vkCreateSwapchainKHR(renderer->device, &info, NULL,
                &renderer->swapchain),
...
static void create_image_views(struct Renderer *renderer)
{
        ...
        vkGetSwapchainImagesKHR(renderer->device, renderer->swapchain,
                &renderer->image_count, images);

        renderer->image_views = malloc(
                renderer->image_count * sizeof(VkImageView));
        ...
        for (uint32_t i = 0; i < renderer->image_count; ++i) {
                ...
                assert_vulkan(vkCreateImageView(renderer->device, &info, NULL,
...
static void create_graphics_pipeline(struct Renderer *renderer)
{
        ...
        assert_vulkan(vkCreateGraphicsPipelines(renderer->device, NULL, 1,
                &info, NULL, &renderer->graphics_pipeline),
